/**
 * extract-beats.mjs — offline beat/onset analysis for the anime-production skill.
 *
 ...
 *
 * Dependency-free by design: decodes with Remotion's own bundled ffmpeg
 * (`npx remotion ffmpeg`), then does spectral-flux onset detection and tempo
 * estimation in plain JS. No Python, no librosa, no extra npm packages.
 *
 * Usage:
 *   node tools/extract-beats.mjs public/bgm/song.mp3 [--out src/data/beats.json]
...
// ---------- 1. decode to mono PCM via Remotion's bundled ffmpeg ----------
/** Remotion ships a platform-specific compositor package containing ffmpeg. */
function findFfmpeg() {
  const pkgs = [
