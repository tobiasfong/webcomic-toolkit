...
This is the canonical rules file for every agent harness (Codex, Cursor,
Gemini, Kimi, Antigravity and the rest read `AGENTS.md`); Claude Code reads it
through a one-line import in `CLAUDE.md`. Edit this file, never that one.
Each server carries its own short `AGENTS.md` pointing at its manual.
...
## Visual novel — where the rules are

The game lives under a gitignored `vn/<project>/`. **The working rules --
the verification sequence, the emitter traps, the sprite audit, the engine
traps, the web build, phones, and how expressions are made -- are the
"Working rules for the game itself" chapter of
`servers/visual-novel-mcp/WORKFLOW.md`.** Read it before touching the game.
Its private supplement, which needs names, is `vn/<project>/HANDOVER.md`.

Four things a session must carry without looking anything up:

1. **Run the whole verification sequence as ONE command** on every change,
   never the steps by hand:

   ```bash
   servers/visual-novel-mcp/.venv/Scripts/python.exe servers/visual-novel-mcp/tools/verify_all.py vn/<project> "<master.docx>"
   ```

   A green run is the only green worth reporting.
2. **Every scene file is GENERATED from the author's document.** Never
   hand-edit one; the next re-emit wipes it. Staging lives in the emitter.
3. **Distrust any checker that shares a definition with what it checks.**
   Three faults shipped because the checker and the thing checked agreed
   with each other while both were wrong. Agreement is not execution.
4. **The author is the gate on every render.** Do not look at images to
   judge them; send them and act on his verdict (see the token economy
   section above).
